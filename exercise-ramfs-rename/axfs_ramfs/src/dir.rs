...
impl VfsNodeOps for DirNode {
    ...
    /// Renames an entry inside this directory tree.
    ///
    /// Only same-directory renames are supported (matching the exercise's
    /// "rename, NOT move" semantics): both `src_path` and `dst_path` must
    /// resolve to the same parent directory; only the final path component
    /// (the entry name) may differ. The entry's underlying node (and thus its
    /// contents) is preserved -- only its key in the in-memory directory map
    /// changes.
    fn rename(&self, src_path: &str, dst_path: &str) -> VfsResult {
        log::debug!("rename at ramfs: {src_path} -> {dst_path}");

        let (src_parent_path, src_name) = rsplit_path(src_path);
        let (dst_parent_path, dst_name) = rsplit_path(dst_path);

        if src_name.is_empty() || dst_name.is_empty() {
            return Err(VfsError::InvalidInput);
        }

        // `Arc<Self>` handle for this directory, used to resolve parent dirs.
        let this = self.this.upgrade().ok_or(VfsError::NotFound)?;

        let resolve = |rel: &str| -> VfsResult<VfsNodeRef> {
            if rel.is_empty() || rel == "." {
                Ok(this.clone() as VfsNodeRef)
            } else {
                this.clone().lookup(rel)
            }
        };

        let src_parent = resolve(src_parent_path)?;
        let dst_parent = resolve(dst_parent_path)?;

        // Only support renaming within the same directory.
        if !Arc::ptr_eq(&src_parent, &dst_parent) {
            log::error!("ramfs rename: cross-directory move is not supported");
...
/// Splits a path into its parent directory part and the final component.
///
